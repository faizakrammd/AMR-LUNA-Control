...
// ── WiFi / UDP ────────────────────────────────────────────────────────────────
const char* WIFI_SSID = "CaptFK17";
const char* WIFI_PASSWORD = "17fkcapt17";

const char* PYTHON_IP = "10.177.58.208";
...
float encA_mm() { noInterrupts(); 
long s=countA; interrupts(); 
...
snprintf(
  ...
  "ToF: %d mm | "
  "EncA: %.2f mm | EncB: %.2f mm | Bot: %.2f mm | "
  "SpdA: %.2f mm/s | SpdB: %.2f mm/s | BotSpd: %.2f mm/s | "
  "Sensors: %s | State: %s",
...
udp.beginPacket(PYTHON_IP, UDP_PORT);
